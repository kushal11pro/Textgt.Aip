...
import { ImageGenInterface } from './components/ImageGenInterface.js';
import { VisionInterface } from './components/VisionInterface.js';
import { FastLab } from './components/FastLab.js';
import { CodeWorkspace } from './components/CodeWorkspace.js';
import { VideoGenInterface } from './components/VideoGenInterface.js';
...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-[#020202] text-[#f8fafc] overflow-hidden select-none">
      ...
      <aside className={`relative h-full border-r border-white/5 bg-[#080808] flex flex-col transition-all duration-700 z-50 ${isMobile ? 'fixed inset-y-0 left-0 w-72' : (isSidebarOpen ? 'w-64' : 'w-0 overflow-hidden')} ${isMobile && !isSidebarOpen ? '-translate-x-full' : 'translate-x-0'}`}>
      ...
      <ApiKeyModal isOpen={isKeyModalOpen} onClose={() => setIsKeyModalOpen(false)} onSuccess={() => setIsKeyModalOpen(false)} />
    </div>
  );
}